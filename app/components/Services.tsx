  ...
  return (
    ...
    >
      ...
      <div className='flex flex-col gap-4'>
        ...
        >
          {tilesData.slice(0, 2).map((x, i) => (
            <PlanningTile key={i} {...x} />
          ))}
        </motion.div>
        ...
        <motion.div
          variants={slideIn("left", "tween", 0.2, 0.5)}
          className='flex flex-col sm:flex-row gap-4 items-center justify-end'
        >
          {tilesData.slice(4, 7).map((x, i) => (
            <PlanningTile key={i} {...x} />
          ))}
        </motion.div>
      </div>
